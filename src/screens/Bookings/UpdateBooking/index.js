...
const Index = props => {
  ...
  const [checkInDate, setCheckInDate] = useState(new Date(item.checkInDate));
  const [checkOutDate, setCheckOutDate] = useState(new Date(item.checkOutDate));
  ...
  const onUpdateBooking = async (data, type, checkInDate, checkOutDate) => {
    try {
      const bookingData = {
        ...data,
        id: bookingId,
      ...
      await dispatch(bookingAction.updateBooking(bookingData));
      setIsLoading(false);
    } catch (error) {
          ...
          <View style={styles.container}>
            <Formik
              ...
              initialValues={{
                ...
                numberAdults: item.numberAdults.toString(),
                numberKids: item.numberKids.toString(),
